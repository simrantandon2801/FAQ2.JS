...
function Contact() {
	...
	return (<>
	  ...
	  <Grid container md={12} lg={12} >
			...
			<Grid container  lg={6}
			md={6} xs={12} sx={{ alignItems: mobile?'initial':'center' }}>
			
				<Grid item md={7.5} xs={12} lg={6} sx={{ mt:mobile?'36px': '42px', marginLeft:mobile?'25px':ipad?'0px':'151px',margin:ipad?'auto':'autopx'}}>
				  <Typography variant='h2' style={{
fontStyle: 'normal',
fontWeight: 600,
fontSize: mobile?'40px':ipad?'32px':'36px',textAlign:'initial',
lineHeight: '48px',marginTop:ipad?'42px':'48px',
color: '#BB0000'}}>We would love to hear from you</Typography> 
				  </Grid>
			<Box component="form" noValidate onSubmit={handleSubmit} sx={{ mt: ipad?'27px':'42px', ml:mobile?'0px':ipad?'0px':'151px' }}>
            <Grid container lg={12} md={7.5} xs={9} sx={{margin:'auto'}}>
			...
			<Grid item md={7} xs={7}>
                ...
                />
						...
						<Grid container md={12} xs={12} sx={{mt:'32px',mb:'10px'}}>
				<Grid item md={8} xs={8} >
...
width: '361px',
height: '460px',marginLeft:'150px',marginTop:ipad?'140px':"-151px",marginBottom:ipad?'222px':'0px'
 }}></Image>}
		...
		<Box sx={{marginLeft:mobile?'18px':'0px',marginTop:mobile?'56px':'50px',marginBottom:"46px"}}> 
		<Card sx={{
			display: 'flex', backgroundImage: 'url(https://drive.google.com/uc?export=view&id=1qZQGUIyH_yaza2IsL4wFX4T1Wx3_qf1q)',backgroundRepeat:"no-repeat",backgroundSize:mobile?'272px':'564px',width: mobile?'272px':'564px', alignItems:'center',borderRadius: '20px',margin:'auto',
height: mobile?'184px': '327px'}}>
	  
			<Box sx={{ display: 'flex', flexDirection: 'column' }}>
			<CardContent sx={{marginLeft:mobile?'60px':"195px"}}>
...
fontSize: mobile?'20px':'36px'}}>Contact us</Typography>
						...
						<Button href={`tel:${phoneNumber}`}  style={{color:"black",fontSize:mobile?'14px':"18px",fontWeight:'600'}} ><LocalPhoneIcon/>+91-7060495034</Button>
					
						<Button  href={`mailto:${emailAddress}`}style={{color:"black",textTransform:'none',fontSize:mobile?'14px':"18px",fontWeight:'600'}} ><EmailIcon />sales@Hubhawks.com</Button>
